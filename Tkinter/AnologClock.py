...
root = tk.Tk()
root.title("Clock")
canvas = tk.Canvas(root, width=WIDTH, height=HEIGHT, bg="black")  # Fond noir
canvas.pack()
...
def update_clock():
    ...
    canvas.create_line(WIDTH/2, HEIGHT/2, hour_x, hour_y, fill="white", width=5)  # Aiguille des heures en blanc
    ...
    canvas.create_line(WIDTH/2, HEIGHT/2, minute_x, minute_y, fill="white", width=3)  # Aiguille des minutes en blanc
   
    second_angle = second * math.pi/30 - math.pi/2
    second_x = WIDTH/2 + 0.7 * WIDTH/2 * math.cos(second_angle)
    second_y = HEIGHT/2 + 0.7 * HEIGHT/2 * math.sin(second_angle)
    canvas.create_line(WIDTH/2, HEIGHT/2, second_x, second_y, fill="red", width=2)  # Aiguille des secondes en rouge
